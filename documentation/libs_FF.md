...
* **appendLI(ul[Node], text[String]) > [Node]**  ___WARNING: susceptible d'être déplacé dans `options_FF.js`___  
...
Retourne une chaine contenant le nombre (ou un encadrement du -) de PX gagnés lors du kill d'une cible dont les infos de niveau sont donnés dans la chaîne `niv`.
...
Calcule les PV restants d'une cible ayant un encadrement de PV donné dans la chaîne `pv`, un pourcentage de blessure `bless` et le met en forme la réponse suivant la valeur de `isVue`.
